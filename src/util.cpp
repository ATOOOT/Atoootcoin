...
const char * const BITCOIN_CONF_FILENAME = "atoootcoin.conf";
const char * const BITCOIN_PID_FILENAME = "atoootcoind.pid";
...
#else
    const char* pszModule = "atoootcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Atoootcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Atoootcoin";
#else
    // Unix
    return pathRet / ".atoootcoin";
#endif
#endif
}
