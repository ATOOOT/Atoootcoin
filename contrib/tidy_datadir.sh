...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete ATOOOT database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no ATOOOT datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old ATOOOT datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected ATOOOT 0.7 datadir."
    ;;
  3)
    echo "Detected ATOOOT pre-0.8 datadir."
    ;;
  4)
    echo "Detected ATOOOT 0.8 datadir."
    ;;
esac
