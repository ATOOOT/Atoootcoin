ATOOOT Core version *0.15.1* is now available from:

  <https://download.ATOOOT.org/ATOOOT-0.15.1/>
...
A number of changes to the way ATOOOT Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 namatooot per kB (0.00001 ATOOOT). This is to prepare for dropping the minimum transaction fee to 0.1 namatooot per kB (0.0001 ATOOOT) in the next release.
